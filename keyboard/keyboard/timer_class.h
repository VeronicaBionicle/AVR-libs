...
class Timer {
  private:
    uint16_t period;
    void setupTimer(uint16_t Period);  //service function for calculating CS and OCR for timers
  public:
    void startTimerForInterrupt(uint16_t Period);  //start timer for attaching function to ISR, period`s unit measure is microseconds
