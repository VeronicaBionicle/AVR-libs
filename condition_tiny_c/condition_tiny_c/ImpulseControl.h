...
void OutputInit (ImpulseControl_t vent); //Initialize output pin
void ZerocrossInit (void); //Initialize zerocross pin
void On (ImpulseControl_t vent); //High level on output pin
void Off (ImpulseControl_t vent); //Low level on output pin
void PhaseControl (ImpulseControl_t vent, uint8_t number_of_periods); //What number of periods are passed from maximum period